...
/* ==========================================================
   V25 MOBILE VIDEO SCROLL FIX
   Extra robust iOS/Android unlock + scroll-scrubbing update.
   ========================================================== */
...
(() => {
  ...
  ['touchstart', 'touchmove', 'pointerdown', 'click'].forEach(eventName => {
    window.addEventListener(eventName, unlockAllVideos, { passive: true, once: true });
  });

  const clampV25 = (value, min, max) => Math.min(max, Math.max(min, value));

  const scrubVideoBySection = (sectionSelector, videoSelector) => {
    ...
    const update = () => {
      const rect = section.getBoundingClientRect();
      const scrollable = Math.max(1, section.offsetHeight - window.innerHeight);
      const progress = clampV25(-rect.top / scrollable, 0, 1);
      const duration = Number.isFinite(video.duration) && video.duration > 0 ? video.duration : 8;
      const maxTime = Math.max(duration - 0.08, 0.1);
      const targetTime = clampV25(progress * maxTime, 0.02, maxTime);

      if (video.readyState > 0) {
        try {
          if (Math.abs(video.currentTime - targetTime) > 0.035 || isMobileLike) {
    ...
    video.addEventListener('loadedmetadata', update, { passive: true });
  ...
  scrubVideoBySection('#hero-scroll', '#scrollVideo');
  scrubVideoBySection('#showcase', '#midScrollVideo');
